...
export default function TimelinePage() {
  ...
  const [form, setForm] = useState({ titulo: '', descricao: '', tipo: 'trabalho', hora: format(new Date(), 'HH:mm') })
  ...
  return (
    <div style={{ background: '#0a0a0a', minHeight: '100dvh', paddingBottom: 40 }}>
      <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 700, margin: 0 }}>Timeline ⏱️</h1>
          <p style={{ color: '#8e8e93', fontSize: 13, margin: '2px 0 0' }}>{format(new Date(), "EEEE, dd 'de' MMMM", { locale: ptBR })}</p>
        </div>
      ...
      <div style={{ padding: '16px' }}>
        {loading ? (
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: 27, top: 0, bottom: 0, width: 2, background: 'rgba(255,255,255,0.06)' }} />
              ...
              {items.map(item => {
                ...
                return (
                  <div key={item.id} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', paddingBottom: 16, position: 'relative' }}>
                    <div style={{ width: 44, height: 44, borderRadius: '50%', flexShrink: 0, zIndex: 1, background: tipo.color + '20', border: `2px solid ${tipo.color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{tipo.icon}</div>
                    <div style={{ flex: 1, background: '#141414', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '12px 14px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          ...
                          <p style={{ color: tipo.color, fontSize: 12, margin: '2px 0 0', fontWeight: 500 }}>{item.hora?.substring(0, 5)} · {tipo.label}</p>
                        </div>
                        <button onClick={() => excluir(item.id)} style={{ color: '#636366', fontSize: 20, background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>×</button>
                      </div>
      ...
      <BottomSheet open={open} onClose={() => setOpen(false)} title="Registrar Atividade">
            ...
            <div className="no-scroll-bar" style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
              {TIPOS_TL.map(t => (
                <button type="button" key={t.id} onClick={() => setForm(f => ({ ...f, tipo: t.id }))}
                  style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5, padding: '8px 14px', borderRadius: 20, cursor: 'pointer', fontSize: 13, fontWeight: 500, background: form.tipo === t.id ? t.color + '30' : 'rgba(255,255,255,0.06)', border: `1.5px solid ${form.tipo === t.id ? t.color : 'transparent'}`, color: form.tipo === t.id ? t.color : '#8e8e93' }}>
                  {t.icon} {t.label}
                </button>
              ))}
            </div>
